        ...
        loop:
          ...
          do:
            io.cloudslang.base.http.http_client_post:
              - url: "${'''%s/bvd-receiver/api/submit/%s/tags/machine_part_level/dims/machine_id,inventory_id''' % (bvd_url,bvd_api_key)}"
              - trust_all_roots: 'true'
              - request_character_set: utf-8
              - body: "${'''{\"machine_id\": \"%s\", \"inventory_id\": \"%s\", \"quantity\": \"%s\"}''' % (machine_id, '%s_part_%s' % (machine_id,part_data.split(',')[0]), part_data.split(',')[2])}"
