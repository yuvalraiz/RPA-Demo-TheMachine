    ...
    - opcmsg:
        do:
          io.cloudslang.base.cmd.run_command:
            - command: "${'''%s msg_grp=%s a=%s o=%s msg_t=\"%s\" sev=%s node=%s -option CIH=%s -option ETI=%s''' % (get_sp('YuvalRaiz.TheMachine.opcmsg'),machine_id,ci,machine_id,msg,sev,node,ci,eti)}"
        navigate:
          - SUCCESS: SUCCESS
          - FAILURE: on_failure
  results:
